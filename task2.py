...
def range_query_tree(k_min=randint(0, 100000), k_max=randint(0, 100000)):
    ...
    # for key, value in tree.items(k_min, k_max):
    #     print(key, value)
...
print("Для 1000 запитів, середнє")
print("Total range_query time for OOBTree:")
print(
    timeit.timeit(
        "range_query_dict()", setup="from __main__ import range_query_dict", number=1000
    )
    / 1000
)
...
print(
    timeit.timeit(
        "range_query_tree()", setup="from __main__ import range_query_tree", number=1000
    )
    / 1000
)
